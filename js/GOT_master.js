(() => {
	...
	const animStark = document.querySelector('.stark');
	const animBaratheon = document.querySelector('.baratheon');
	const animLannister = document.querySelector('.lannister');
	const animTully = document.querySelector('.tully');
	const animGreyjoy = document.querySelector('.greyjoy');
	const animArryn = document.querySelector('.arryn');
...
`House Frey of the Twins was the Great House of the Riverlands, having gained their position for their treachery against their former liege lords, House Tully, who were stripped of all their lands and titles for their rebellion against the Iron Throne; House Tully had supported the independence movement for the Kingdom of the North. The current head of the house is unknown following the assassinations of Lord Walder Frey and two of his sons, Lothar Frey and Walder Rivers, by the vengeful Arya Stark. This is made more complex by the subsequent assassination of all the male Freys soon after.`]
	...
	const houseDataTitle = [ [`Stark`], [`Baratheon`], [`Lannister`], [`Tully`], [`Greyjoy`], [`Arryn`]
	];
	...
	function openLightbox() {
		...
		setTimeout(function(){lightbox.classList.add('lightbox-on'); }, 1000);
		video.load();
		video.play();
	...
	function animateBanners() {
		...
		// TweenMax.to(topBanners, 0.8, { right: currentOffset });
		houseInfo.innerHTML = houseData[this.dataset.offset];
		tagline.innerHTML = houseDataTitle[this.dataset.offset];
	...
	function animateIcon() {
		TweenMax.to(animStark, 0.7, {scaleX: 1.5, scaleY: 1.5, ease:  SlowMo.easeOut})};

	function animBaratheonIcon() {
		TweenMax.to(animBaratheon, 0.7, {scaleX: 1.5, scaleY: 1.5, ease:  SlowMo.easeOut})};

	function animLannisterIcon() {
		TweenMax.to(animLannister, 0.7, {scaleX: 1.5, scaleY: 1.5, ease:  SlowMo.easeOut})};

	function animTullyIcon() {
		TweenMax.to(animTully, 0.7, {scaleX: 1.5, scaleY: 1.5, ease:  SlowMo.easeOut})};

	function animGreyjoyIcon() {
		TweenMax.to(animGreyjoy, 0.7, {scaleX: 1.5, scaleY: 1.5, ease:  SlowMo.easeOut})};
	 function animArrynIcon()  {
	 	TweenMax.to(animArryn, 0.7, {scaleX: 1.5, scaleY: 1.5, ease:  SlowMo.easeOut})};
...
sigils.forEach(sigil => sigil.addEventListener('click', openLightbox, false));

// animation
animStark.addEventListener('click', animateIcon);
animBaratheon.addEventListener('click',animBaratheonIcon );
animLannister.addEventListener('click', animLannisterIcon);
animTully.addEventListener('click', animTullyIcon );
animGreyjoy.addEventListener('click', animGreyjoyIcon);
animArryn.addEventListener('click', animArrynIcon );
